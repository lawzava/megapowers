...
- One-shot: `claude -p "<prompt>"`. Pin the model from the resolved route with
  `--model <id>`. For machine-checkable output add `--output-format json` and
  state the required JSON shape in the prompt.
