...
# is under 440 words, that the dead-weight blocks are stripped (YAML frontmatter,
...
# and that the workflow-critical Core Rule survives. Ceiling 440, raised from 385
# when the always-on Scripting rule landed. Keep all three numbers in step; the
...
if [ "${words:-0}" -le 440 ]; then pass=$((pass + 1)); else fail=$((fail + 1)); printf '  FAIL emitted payload %s words, want <=440\n' "$words"; fi
