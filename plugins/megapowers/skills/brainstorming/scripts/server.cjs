...
const MEGAPOWERS_VERSION = readMegapowersVersion();
...
function readMegapowersVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Manifest missing or unreadable; try the next one.
    }
  }
...
// Offline branding: plain text only, no network calls.
function brandMarkup() {
  const version = escapeHtmlText(MEGAPOWERS_VERSION);
  return '<div class="brand"><span class="brand-copy">megapowers v' + version + '</span></div>';
}
...
function wrapInFrame(content) {
  // Function replacer so `$`-sequences in content ($', $`, $&, $$) are inserted
