...
Browser-based visual brainstorming companion for showing mockups, diagrams, and options. It runs entirely on localhost and makes no network calls.
...
#           "screen_dir":"/path/to/project/.megapowers/brainstorm/12345-1706000000/content",
#           "state_dir":"/path/to/project/.megapowers/brainstorm/12345-1706000000/state"}
```
...
**Finding connection info:** The server writes its startup JSON to `$STATE_DIR/server-info`. If you launched the server in the background and didn't capture stdout, read that file to get the URL and port. When using `--project-dir`, check `<project>/.megapowers/brainstorm/` for the session directory.

**Note:** Pass the project root as `--project-dir` so mockups persist in `.megapowers/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.megapowers/` to `.gitignore` if it's not already there.
...
1. **Check server is alive**, then **write HTML** to a new file in `screen_dir`:
   ...
   - Use your file-creation tool — don't use cat/heredoc (dumps noise into terminal)
...
If the session used `--project-dir`, mockup files persist in `.megapowers/brainstorm/` for later reference. Only `/tmp` sessions get deleted on stop.
