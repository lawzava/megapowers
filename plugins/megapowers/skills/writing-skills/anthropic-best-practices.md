...
Note — YAML frontmatter: the SKILL.md frontmatter requires two fields:
...
A basic Skill starts with just a SKILL.md file containing metadata and instructions (YAML frontmatter plus a markdown body).

As your Skill grows, you can bundle additional content that agents load only when needed (for example, reference.md and forms.md alongside SKILL.md).
...
Note: this example demonstrates a data-driven evaluation with a simple testing rubric. We do not currently provide a built-in way to run these evaluations. Users can create their own evaluation system. Evaluations are your source of truth for measuring Skill effectiveness.
...
The sections below focus on Skills that include executable scripts. If your Skill uses only markdown instructions, skip to the Checklist for effective Skills.
