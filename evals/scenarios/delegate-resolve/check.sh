...
# Effort is medium here on purpose and this is not a weakened assertion: the
# 2026-08-05 audit routed roles bounded by something other than reasoning depth at
# medium, and visual_verify is bounded by the browser driver and the evidence it
# captured. The tier is the part that carries the judgement, so it stays frontier.
awk '/=== visual_verify ===/{f=1} /=== unknown ===/{f=0} f&&/PROVIDER=claude/{p=1} f&&/DRIVER_BINARY=playwright-cli/{d=1} f&&/TIER=frontier/{t=1} f&&/EFFORT=medium/{e=1} END{exit !(p&&d&&t&&e)}' "$o" || { echo "visual_verify did not resolve to a frontier vision judge plus playwright driver"; exit 1; }
