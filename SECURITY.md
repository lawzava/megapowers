...
| `mega-guardrails` | `deny-destructive` (PreToolUse: Bash); `auto-format` (PostToolUse: Write/Edit) | deny-destructive reads the proposed command on stdin and writes an allow/ask/deny decision; auto-format reads the just-written file path and reformats that one file (`gofmt`/`goimports`/`prettier`) | none (hooks and an optional statusline only) | none |
